...
char strings[max_strings][max_strlen];
int cur_string = 0;

char* alloc_string() {
	if(cur_string == max_strings) return nullptr;
...
bool get_key(const char* wr, int len, token* tok) {
	// expect PRINT | IF | THEN | GOTO | INPUT | LET | CLEAR | LIST | RUN | END
...
void list_tokens() {
	// go through lines and print tokens
...
bool get_tok_val(token tok, int& res) {
	// if it's a number, give it
	if(tok.type == T_NUM) {
		res = tok.payload.num;
		return true;
	}

	// if it's a variable, check if defined and give it
	if(tok.type == T_VAR) {
...
void apply_aritmop(int& res, int arg, op_type type) {
	...
	// if no such operation type do nothing
}
...
bool apply_relop(int arg1, int arg2, op_type type) {
	...
	return false; // never reached, better safe than sorry
}
...
bool eval_expr(token*& toks, int& res, bool continues = true) {
	...
	while(toks->type != T_MARK) {
		if(!is_aritmop(*toks)) {
			if(continues) {
				// might continue, move to previous token and return (for next parsing function) 
				toks--;
				return true;
			} else {
...
bool exec_if(token* toks, int* line) {
	...
	// check here (early) if if goes through
...
bool exec_statement(token* toks, int* line) {
	...
	switch(toks[0].payload.key) {
		...
		case(IF): 		return exec_if(toks + 1, line);		// these need a reference to the line number
		case(GOTO): 	return exec_goto(toks + 1, line);	// as they might change it
